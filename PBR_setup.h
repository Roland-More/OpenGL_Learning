// TODO: Function should be split
// into framebuffers and textures
...
struct PBR_setup {
    ...
    unsigned int gPositionMetallic;
    unsigned int gNormalRoughness;
    unsigned int gAlbedoAo;
...
PBR_setup PBR_deferredSetup3x4f(const float width, const float height, const char* environmentTexturePath)
{
    ...
    unsigned int gPositionMetallic, gNormalRoughness, gAlbedoAo;
    ...
    // Cleanup openGL objects no longer of use
    // Shaders have automatic destructors


    return {gBuffer, gPositionMetallic, gNormalRoughness, gAlbedoAo, irradianceMap, prefilterMap, brdfLUTTexture, envCubemap};
}
